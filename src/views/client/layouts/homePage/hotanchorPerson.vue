<template>
  <div class="anchorpersonMain">
    <p class="titelCommnt anchorpersontitle">热门主播</p>
    <div class="anChorPersonTranslate">
      <div class="topAnchorPerson">
        <ul v-for="(item,index) in hotpersonListlen" :key="index" :class="{transformNew:index%2!=0}">
          <li v-for="(item,index) in anchorPerson" :key="index"
           class="opcatityHotPerson"
           :class="{opcatityAnchorPersonshow:index<=indexHotperson}">
...
import { onMounted, reactive, ref,onBeforeUnmount } from 'vue';
  const anchorPerson=reactive([{},{},{},{},{},{}])
  const hotpersonListlen=reactive([{},{},{},{}])
  let indexHotperson=ref(0);
  const setTimeHotPerson=ref()
  onMounted(()=>{
    opcatityAnchorPerson()
  })
  const opcatityAnchorPerson=(()=>{
    setTimeHotPerson.value=setInterval(()=>{
        indexHotperson.value++;
        if(indexHotperson.value==5){
            indexHotperson.value=0
        }
    },1000)
  })
  onBeforeUnmount(()=>{
    setTimeHotPerson.value=null
  }) 
</script>
  <style lang="scss">
@import "@/views/client/styles/homePage/anchorPerson.scss";
